...
public class MemberController {
	...
	public void insertMember(String userId, String userPwd, String userName, String gender,
		...
		// view로부터 전달받은 값을 바로 dao쪽으로 전달하는 것이 아니라
		// 어딘가(Member객체)에 주섬주섬 담아서 전달
		
		// 방법1) 기본생성자로 생성한 후 각필드에 setter메소드를 통해서 일일히 담는 방법
		// 방법2) 매개변수 생성자로 생성과 동시에 담는 방법
		Member m = new Member(userId, userPwd, userName, gender, Integer.parseInt(age), email, phone, address, hobby);
		// 문자열 => 기본자료형으로 변경하는 것을 "파싱"
		// String => int
		...
		if(result > 0) { //성공
			new MemberMenu().displaySuccess("성공적으로 회원추가되었습니다.");
		} else { //실패
	...
	public void selectList(){
		...
		// 조회결과가 있는지 없는지 판단 한 후 사용자가 보게 될 응답 화면 지정
		if(list.isEmpty()) { // 텅비어있을 경우 == 조회된 데이터 없을 경우
	...
	public void selectByUserName(String keyword) {
		...
		if(list.isEmpty()) { // 텅 빈 리스트일 경우 => 검색 결과 없음
			new MemberMenu().displayNoData(keyword + "에 해당하는 검색 결과가 없습니다.");
		} else { // 그게 아닐 경우 => 검색 결과 없음
			new MemberMenu().displayMemberList(list);
		}
